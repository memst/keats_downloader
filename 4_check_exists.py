...
def check_exists(database):
    ...
    for (page_url,) in database.execute("SELECT pageUrl FROM Videos WHERE videoUrl IS NOT NULL"):
        print(page_url)
        paths = kd_utilities.get_paths(page_url, database, online=False)

        if os.path.isfile(paths['file_path']):
            database.execute("UPDATE Videos SET file_exists = 1 WHERE pageUrl = ?", (page_url,))
